...
| `REPORT_LANGUAGE` | 报告输出语言：`zh`(默认中文) / `en`(英文)；会同步影响个股分析 Prompt、大盘复盘 Prompt、模板、通知 fallback 与 Web 报告页固定文案 | 可选 |
| `LOG_LANGUAGE` | 运行日志语言：`zh` / `en` / `follow_report`；默认 `zh`，GitHub Actions 建议设为 `follow_report` | 可选 |
