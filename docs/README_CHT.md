...
| `REPORT_LANGUAGE` | 報告輸出語言：`zh`(預設中文) / `en`(英文)；會同步影響個股分析 Prompt、大盤復盤 Prompt、Markdown 模板、通知 fallback 與 Web 報告頁固定文案 | 可選 |
| `LOG_LANGUAGE` | 執行期日誌語言：`zh` / `en` / `follow_report`；設為 `follow_report` 時會跟隨 `REPORT_LANGUAGE` | 可選 |
