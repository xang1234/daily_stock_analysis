...
3. Register routes and exception handlers
...
def create_app(static_dir: Optional[Path] = None) -> FastAPI:
    ...
    Args:
        static_dir: Optional static directory path. Defaults to the project's
            `static` directory.
    ...
    app = FastAPI(
        ...
        description=(
            "AI stock analysis API for A-shares, Hong Kong stocks, and US stocks\n\n"
            "## Modules\n"
            "- Stock analysis: trigger AI-powered analysis\n"
            "- History: query saved analysis reports\n"
            "- Market data: retrieve quote data\n\n"
            "## Authentication\n"
            "Optional runtime authentication can be enabled or disabled from the Web UI settings page"
        ),
        version="1.0.0",
    ...
    # Add extra allowed origins from environment variables
    extra_origins = os.environ.get("CORS_ORIGINS", "")
    if extra_origins:
        allowed_origins.extend([o.strip() for o in extra_origins.split(",") if o.strip()])
    
    # Allow all origins for development / demo usage
    ...
    if has_frontend:
        ...
        async def root():
            """Root route: return the frontend page."""
...
<p class="status">API Version 1.0.0 &bull; <a href="/api/health">/api/health</a></p>
        ...
        @app.get("/", include_in_schema=False)
        async def root():
            """Root route: return a helper page when the frontend is not built."""
    ...
    @app.get(
        ...
        description="Used by load balancers and monitoring systems to verify service health"
    )
