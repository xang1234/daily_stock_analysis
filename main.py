...
def run_full_analysis(
    ...
    try:
        ...
        if merge_notification and (results or market_report) and not args.no_notify:
            ...
            parts = []
            if market_report:
                parts.append(f"# 📈 {labels['market_review_title']}\n\n{market_report}")
            if results:
                dashboard_content = pipeline.notifier.generate_aggregate_report(
                    results,
                    getattr(config, 'report_type', 'simple'),
                )
                parts.append(f"# 🚀 {labels['stock_dashboard_merged_title']}\n\n{dashboard_content}")
            if parts:
                combined_content = "\n\n---\n\n".join(parts)
        ...
        if results:
            ...
            for r in sorted(results, key=lambda x: x.sentiment_score, reverse=True):
                emoji = r.get_emoji()
                display_name = get_localized_stock_name(r.name, r.code, report_language)
                logger.info(
                    f"{emoji} {display_name}({r.code}): {localize_operation_advice(r.operation_advice, report_language)} | "
                    f"Score {r.sentiment_score} | {localize_trend_prediction(r.trend_prediction, report_language)}"
                )
        ...
        try:
            ...
            if feishu_doc.is_configured() and (results or market_report):
                ...
                doc_title = f"{now.strftime('%Y-%m-%d %H:%M')} {labels['market_review_doc_title']}"
                ...
                if market_report:
                    full_content += f"# 📈 {labels['market_review_title']}\n\n{market_report}\n\n---\n\n"
                ...
                if results:
                    ...
                    full_content += f"# 🚀 {labels['stock_dashboard_merged_title']}\n\n{dashboard_content}"
                ...
                if doc_url:
                    ...
                    if not args.no_notify:
                        success_text = (
                            f"[{now.strftime('%Y-%m-%d %H:%M')}] Market review document created: {doc_url}"
                            if report_language == "en"
                            else f"[{now.strftime('%Y-%m-%d %H:%M')}] 复盘文档创建成功: {doc_url}"
                        )
                        pipeline.notifier.send(success_text)
